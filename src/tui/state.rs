...
use crate::frontend::TableOption;
...
pub struct TableActionsDialog {
    ...
    pub vertical_scroll_state: ScrollbarState,
    pub selected: usize,
    pub actions: Vec<TableOption>,
}

impl TableActionsDialog {
    pub fn new(selected: usize, actions: Vec<TableOption>) -> Self {
        Self {
            items: ListState::default(),
            vertical_scroll_state: ScrollbarState::default().content_length(actions.len()),
            selected,
            actions,
        }
    }
...
impl TableList {
    ...
    fn sort_by_lastmodified(&mut self) {
        ...
        self.items
            .sort_by_key(|t| std::cmp::Reverse(t.last_modified));
        self.select_first();
    }
...
impl State {
    ...
    pub(crate) fn open_dialog(&mut self) {
        if let Some(selected) = self.tables.state.selected() {
            let actions = TableOption::all(&self.config);
            let mut dialog = TableActionsDialog::new(selected, actions);
